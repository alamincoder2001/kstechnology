...
class Page extends CI_Controller {
    ...
    public function branddelete(){
        ...
        try {
            $data = json_decode($this->input->raw_input_stream);
            $brands = $this->db->query("select image from tbl_brand where brand_SiNo = $data->brandId")->row();
            $imagePath = './uploads/brands/'. $brands->image; 
            if(file_exists($imagePath)){
                unlink($imagePath);
            }
            $brands = $this->db->query("update tbl_brand set status = 'd' where brand_SiNo = $data->brandId");
