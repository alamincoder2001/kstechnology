...
class Products extends CI_Controller {
    ...
    public function addProduct() {
        ...
        try{
            ...
            // $productObjimageMultiple = json_decode($this->input->raw_input_stream);
            ...
            $productCodeCount = $this->db->query("select * from tbl_product where Product_Code = ?", $productCode)->num_rows();
            if($productCodeCount != 0){
            ...
            if(!empty($_FILES['image'])) {
                ...
                $imageName = $productObj->Product_Code;
                ...
                $imageName = $productObj->Product_Code . $this->upload->data('file_ext');

                $this->db->query("update tbl_product set image = ? where Product_SlNo = ?", [$imageName, $productId]);
            }
    ...
    public function updateProduct(){
        ...
        try{
            ...
            if(!empty($_FILES['image'])) {
                ...
                $imageName = $productObj->Product_Code . $this->upload->data('file_ext');
                $this->db->query("update tbl_product set image = ? where Product_SlNo = ?", [$imageName, $productId]);
                $imageUnlink = './uploads/products/'. $oldImage; 
                if($oldImage != null && file_exists($imageUnlink)){
                    unlink($imageUnlink);
                }
            }
    ...
    public function getTotalStock(){
        ...
        if(isset($data->date)){
            $format_date = $data->date ." 23:11:59";
            //print_r($format_date);
        }
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(od.quantity), 0) 
                    ...
                    join orders os on os.id = od.order_id
                    where od.product_id = p.Product_SlNo and os.status = 'd'                                      
                    " . (isset($data->date) && $data->date != null ? " and os.created_at <= '$format_date' " : "") . "
                ) as sold_quantity_website,
                ...
                (select (purchased_quantity + sales_returned_quantity + transferred_to_quantity) - (sold_quantity + sold_quantity_website + purchase_returned_quantity + damaged_quantity + transferred_from_quantity)) as current_quantity,
    ...
    public function getProductLedger(){
        ...
        $result = $this->db->query("
            ...
            select 
                ...
                concat('Sale -Software- ', sm.SaleMaster_InvoiceNo, ' - ', c.Customer_Name) as description,
            ...
            select 
                ...
                concat('Sale - Website-', o.invoice_no, ' - ', c.name) as description,
                od.price as rate,
                0 as in_quantity,
                od.quantity as out_quantity
            from order_details od
            join orders o on o.id = od.order_id
            join customers c on c.id = o.customer_id
            where od.product_id = " . $data->productId . "
            and o.status = 'd'
        ...
        $previousRows = array_values(array_filter($ledger, function($row) use ($data){
