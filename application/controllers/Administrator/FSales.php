...
class FSales extends CI_Controller
{
    ...
    public function index($serviceOrProduct = 'product')
    {
        ...
        $data['title'] = "Product FSales";

        $invoice = $this->mt->generateFSalesInvoice();

        $data['isService'] = $serviceOrProduct == 'product' ? 'false' : 'true';
        $data['salesId'] = 0;
        $data['invoice'] = $invoice;
        $data['content'] = $this->load->view('Administrator/sales/product_fsales', $data, TRUE);
    ...
    public function addSales()
    {
        ...
        try {
            ...
            $invoiceCount = $this->db->query("select * from tbl_salesmaster_false where SaleMaster_InvoiceNo = ?", $invoice)->num_rows();
            if ($invoiceCount != 0) {
                $invoice = $this->mt->generateFSalesInvoice();
            }
            ...
            if (isset($data->customer)) {
                ...
                $customer['Customer_Code']     = $this->mt->generateFCustomerCode();
                $customer['status']            = 'a';
                ...
                $this->db->insert('tbl_customer_false', $customer);
            ...
            $this->db->insert('tbl_salesmaster_false', $sales);
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                $this->db->insert('tbl_saledetails_false', $saleDetails);
            }
    ...
    public function salesEdit($productOrService, $salesId)
    {
        ...
        $sales = $this->db->query("select * from tbl_salesmaster_false where SaleMaster_SlNo = ?", $salesId)->row();
    ...
    public function getSales()
    {
        ...
        $sales = $this->db->query("
                ...
                e.Employee_Name,
                br.Brunch_name
                from tbl_salesmaster_false sm
                left join tbl_customer_false c on c.Customer_SlNo = sm.SalseCustomer_IDNo
                left join tbl_employee e on e.Employee_SlNo = sm.employee_id
                left join tbl_brunch br on br.brunch_id = sm.SaleMaster_branchid
                where sm.SaleMaster_branchid = '$branchId'
                and sm.Status = 'a'
                ...
                'b' as sequence,
                'website' as by_sale,
                o.id as SaleMaster_SlNo,
                o.invoice_no as SaleMaster_InvoiceNo,
                SUBSTRING(o.created_at, 1, 10) as SaleMaster_SaleDate,
                o.customer_id as SalseCustomer_IDNo,
                '' as AddBy,
                '' as SaleMaster_SaleType,
                (o.total_amount - o.shipping_cost) as SaleMaster_SubTotalAmount,
                0.00 as SaleMaster_TaxAmount,
                0.00 as SaleMaster_TotalDiscountAmount,
                o.shipping_cost as SaleMaster_Freight,
                o.total_amount as SaleMaster_TotalSaleAmount,
                o.total_amount as SaleMaster_PaidAmount,
                0.00 as SaleMaster_DueAmount,
                o.cus_message as SaleMaster_Description,               
                concat(o.invoice_no, ' - ', c.name) as invoice_text,
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            if (isset($data->customer)) {
                ...
                $this->db->where('Customer_SlNo', $data->sales->customerId)->update('tbl_customer_false', $customer);
            }
            ...
            $this->db->update('tbl_salesmaster_false', $sales);

            // $currentSaleDetails = $this->db->query("select * from tbl_saledetails where SaleMaster_IDNo = ?", $salesId)->result();
            $this->db->query("delete from tbl_saledetails_false where SaleMaster_IDNo = ?", $salesId);
    ...
    function false_sales_record()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "FSales Record";
        $data['content'] = $this->load->view('Administrator/sales/fsales_record', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function saleInvoicePrint($saleId)
    {
        $data['title'] = "Sales Invoice";
        $data['salesId'] = $saleId;
        $data['content'] = $this->load->view('Administrator/sales/fsellAndreport', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function  deleteSales()
    {
        ...
        try {
            ...
            $sale = $this->db->select('*')->where('SaleMaster_SlNo', $saleId)->get('tbl_salesmaster_false')->row();
            if ($sale->Status != 'a') {
            ...
            // $saleDetails = $this->db->select('Product_IDNo, SaleDetails_TotalQuantity')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails_false')->result();
            ...
            /*Delete Sale Details*/
            $this->db->set('Status', 'd')->where('SaleMaster_IDNo', $saleId)->update('tbl_saledetails_false');

            /*Delete Sale Master Data*/
            $this->db->set('Status', 'd')->where('SaleMaster_SlNo', $saleId)->update('tbl_salesmaster_false');
