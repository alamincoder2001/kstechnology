...
<script>
	...
	new Vue({
		...
		methods: {
			...
			async getCustomers() {
				await axios.post('/get_fcustomers', {
			...
			async saveSales() {
				...
				axios.post(url, data).then(async res => {
					...
					if (r.success) {
						...
						if (conf) {
							window.open('/fsale_invoice_print/' + r.salesId, '_blank');
							await new Promise(r => setTimeout(r, 1000));
							window.location = this.sales.isService == 'false' ? '/fsales/product' : '/fsales/service';
						} else {
							window.location = this.sales.isService == 'false' ? '/fsales/product' : '/fsales/service';
						}
					} else {
